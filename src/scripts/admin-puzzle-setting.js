  ...
  api("get", "admin/check", undefined, (res) => {
    if (res) {
      if (res.msg && res.msg == "ERROR") {
        ...
        api("get", "puzzle_opt", undefined, (res) => {
          if (res.msg == "OK") {
            ...
            for (const key in data) {
              if (Object.hasOwnProperty.call(data, key)) {
                if (key != "_id" && key != "__v" && key != "assets") {
                  document.getElementById(`${key}Txt`).value =
                    data[key].join(",");
                }
              }
            }
            ...
            document.getElementById("saveBtn").onclick = () => {
              api(
                ...
                `puzzle_opt/${data._id}`,
                {
                  score: document.getElementById("scoreTxt").value.split(","),
                },
                (res) => {
                  if (res.msg == "OK") {
                    alert("저장 되었습니다.");
        ...
        document.addEventListener("change", (e) => {
          ...
          if (target.type == "file") {
            ...
            api("post", `upload/puzzle/${target.name}`, formData, (res) => {
              if (res.msg == "ERROR") {
        ...
        document.getElementById("scoreTxt").setAttribute("type", "text");
