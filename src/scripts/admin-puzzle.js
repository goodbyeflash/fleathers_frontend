...
function onLoadTable() {
  ...
  const filter = window.sessionStorage.getItem("puzzle_filter");
  let method = type == "find" || filter ? "post" : "get";
  let url = type == "find" || filter ? "puzzle/find" : "puzzle/";
        ...
        for (let index = 0; index < userItems.length; index++) {
          document.getElementById(`clear_${index}`).onclick = (e) => {
            if (window.confirm("초기화 하시겠습니까?")) {
              api(
                "patch",
                `puzzle/${e.target.getAttribute("data-val")}`,
                {
                  score: 0,
                },
                (res) => {
...
function onSerachData() {
  ...
  pageCount = 1;
  type = "find";
  window.sessionStorage.setItem("puzzle_filter", JSON.stringify(data));
  onLoadTable();
}

function onSerachClear() {
  window.sessionStorage.clear("puzzle_filter");
  document.getElementById("findText").value = "";
  pageCount = 1;
  data = {};
  type == "all";
